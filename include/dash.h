        ...
        class Dashboard {
            ...
            updateGauge(type, value, max) {
                ...
                if (valueEl && fillEl) {
                    ...
                    // Get the appropriate color for this gauge type
                    ...
                    // Update gauge fill based on percentage
                    this.updateGaugeFill(fillEl, percentage, color);
                }
            }
            
            updateGaugeFill(fillEl, percentage, color) {
                // Reset all borders
                ...
                fillEl.style.borderLeftColor = 'transparent';
                
                // Fill the gauge based on percentage (0-100%)
                if (percentage > 0) {
                    if (percentage <= 12.5) { // First 1/8 (bottom)
                        fillEl.style.borderBottomColor = color;
                    } else if (percentage <= 25) { // Second 1/8 (bottom-left)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                    } else if (percentage <= 37.5) { // Third 1/8 (left)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                    } else if (percentage <= 50) { // Fourth 1/8 (top-left)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                        fillEl.style.borderTopColor = color;
                    } else if (percentage <= 62.5) { // Fifth 1/8 (top)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                        fillEl.style.borderTopColor = color;
                    } else if (percentage <= 75) { // Sixth 1/8 (top-right)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                        fillEl.style.borderTopColor = color;
                        fillEl.style.borderRightColor = color;
                    } else if (percentage <= 87.5) { // Seventh 1/8 (right)
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                        fillEl.style.borderTopColor = color;
                        fillEl.style.borderRightColor = color;
                    } else { // Eighth 1/8 (bottom-right) - full circle
                        fillEl.style.borderBottomColor = color;
                        fillEl.style.borderLeftColor = color;
                        fillEl.style.borderTopColor = color;
                        fillEl.style.borderRightColor = color;
